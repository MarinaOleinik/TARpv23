...
def change_color():
    tahvel.itemconfig(red_light,xcor=50, ycor=50, xcor=150, ycor=150, fill="gray")
    tahvel.itemconfig(yellow_light, 50, 180, 150, 280,fill="gray")
    tahvel.itemconfig(green_light,50, 310, 150, 410, fill="gray")
...
#        valgusfoor()
    # Рисуем красный сигнал светофора
red_light=tahvel.create_oval(0,0,0,0, fill="gray")
    # Рисуем желтый сигнал светофора
yellow_light = tahvel.create_oval(0,0,0,0, fill="gray")
    # Рисуем зеленый сигнал светофора
green_light = tahvel.create_oval(0,0,0,0, fill="gray")
estonia_radio = Radiobutton(raam, text="Флаг Эстонии", variable=item_var, value="estonia", command=draw_estonia_flag)
estonia_radio.pack()

bahamas_radio = Radiobutton(raam, text="Флаг Багамских островов", variable=item_var, value="bahamas", command=draw_bagami_flag)
bahamas_radio.pack(pady=5)

russia_radio = Radiobutton(raam, text="Флаг Франции", variable=item_var, value="prantsusmaa", command=draw_prantsusmaa_flag)
russia_radio.pack(pady=5)

rainbow_radio = Radiobutton(raam, text="радужное_переливание", variable=item_var, value="rainbow_ball", command=draw_rainbow_gradient)
rainbow_radio.pack(pady=5)

square_radio = Radiobutton(raam, text="бесконечные квадратики и кружочки", variable=item_var, value="square_in_square", command=draw_estonia_flag)
square_radio.pack(pady=5)

chessboard_radio = Radiobutton(raam, text="Шахматы", variable=item_var, value="chessboard", command=change_color)
chessboard_radio.pack(pady=5)

valgusfoor_radio = Radiobutton(raam, text="Светофор", variable=item_var, value="valgusfoor", command=valgusfoor)
